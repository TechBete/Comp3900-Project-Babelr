...
import ProfilePage from "../pages/Listener/profile"; // Adjust if the path is different
...
describe("Listener Profile Page", () => {
    it("renders profile page and user data correctly", async () => {
      (fetch as jest.Mock).mockResolvedValueOnce({
        ...
        json: async () => ({
          ...
          Password: "********",
          "Reward Points": 50,
          "Background Info": "I love linguistics",
      ...
      expect(await screen.findByText("I love linguistics")).toBeInTheDocument();
  
      // Language fields
      expect(await screen.findByText("English")).toBeInTheDocument();
      expect(await screen.findByText("Proficiency: Native")).toBeInTheDocument();
      expect(await screen.findByText("French")).toBeInTheDocument();
      expect(await screen.findByText("Proficiency: Professional")).toBeInTheDocument();
    });
    ...
    it("opens personal info dialog when clicking Edit Info", async () => {
        ...
        expect(screen.getByLabelText(/background info/i)).toBeInTheDocument();
      });
});